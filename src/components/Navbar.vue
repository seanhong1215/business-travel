  ...
  >
    ...
    <div v-if="length !== 0" class="offcanvas-body">
      <div class="card mb-3" v-for="item in cart.carts" :key="item.id">
        <div class="row g-0">
          ...
          <div class="col-md-3 input-space">
            <p class="card-text">NT${{ item.total }}</p>
          </div>
      ...
      <div class="btn-wrap text-center">
        <button
          class="btn btn-primary btn-lg text-white w-100 mb-3"
          aria-label="Close"
          data-bs-dismiss="offcanvas"
          @click="$router.push({ path: `/shoppingCart` })"
        >
          查看購物車
        </button>
        <button class="btn btn-outline-danger" @click="deleteProducts">
          清空購物車
        </button>
      </div>
    </div>
